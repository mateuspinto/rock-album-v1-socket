...
def community_market__get_sticker_price(request, database):
    DB_CUR = database.cursor()
    QUERRY_RESULT = list(DB_CUR.execute(f'SELECT price FROM stickers WHERE price=(SELECT MIN(price) FROM stickers WHERE name="{request["sticker_name"]}" AND is_for_sale=1)'))
